...
namespace Raysist
{
    class GameScene : Scene
    {
        ...
        {
            base.Update();
            if (DX.CheckHitKey(DX.KEY_INPUT_E) == 1)
            {
                Game.Instance.SceneController.ChangeScene<ClearScene, LoadScene>();
            }
            if (DX.CheckHitKey(DX.KEY_INPUT_O) == 1)
            {
                Game.Instance.SceneController.ChangeScene<GameOverScene, LoadScene>();
            }
        }
    }
}
