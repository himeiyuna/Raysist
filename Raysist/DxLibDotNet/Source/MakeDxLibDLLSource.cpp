...
//	if( strcmp( Str, "COLORDATA" ) == 0 ) return true;
	...
	{
		// IPDATA のポインタと void 型ポインタは unsafe のみ有効
...
int TypeConv( TextAnalyse *txt, char *CSType, bool UseSize, char *CPPType, bool *pOutFlag, bool Struct, bool Unsafe )
{
	...
	for(;;)
	{
		...
		if( strcmp( Str1, "const" ) == 0 )
		{
			...
			if( strcmp( Str2, "void" ) == 0 && strcmp( Str3, "*" ) == 0 )
			{
				txt->getstr( Str4 );
				txt->getstr( Str4 );
				txt->getstr( Str4 );
				sprintf( CPPType, "%s %s %s", Str1, Str2, Str3 );
				strcpy( CSType, "void * " );
				return 4;
			}

			// const IMEINPUTCLAUSEDATA * の場合は特殊なのでここで処理してしまう
			if( strcmp( Str2, "IMEINPUTCLAUSEDATA" ) == 0 && strcmp( Str3, "*" ) == 0 )
			{
				txt->getstr( Str4 );
				txt->getstr( Str4 );
				txt->getstr( Str4 );
			...
			{
				if( strcmp( Str3, "*" ) == 0 && strcmp( Str4, "*" ) == 0 )
				{
					txt->getstr( Str4 );
					txt->getstr( Str4 );
					txt->getstr( Str4 );
					txt->getstr( Str4 );
					sprintf( CPPType, "%s char %s %s", Str1, Str3, Str4 );
					strcpy( CSType, "uint " );
					return 4;
				}
				if( strcmp( Str3, "*" ) == 0 )
				{
					...
					txt->getstr( Str4 );
					txt->getstr( Str4 );
					sprintf( CPPType, "%s char %s ", Str1, Str3 );
				...
				for( l = 0; l < 2; l++ )
				{
					...
					for( i = 0; i < Func.ParameterNum; i ++, par ++ )
					{
						...
						else
						{
							...
							if( par->Out )
							{
								// out 属性でも System.Text.StringBuilder か unsafe の場合はそのまま変数名を出力
